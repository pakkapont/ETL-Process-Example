...
API = Variable.get("API") # get values on keys
MYSQL_CONNECTION = "mysql_default"   # ชื่อของ connection MySQL ใน Airflow ที่เซ็ตเอาไว้
CONVERSION_RATE_URL = API

# path ที่จะใช้ (google cloud storage)
...
with DAG(
    ...
    t4 = BashOperator(
        ...
        bash_command = "bq load --source_format=CSV \
            --autodetect workshop5.audible_data \
                gs://asia-east2-workshop5-8b0e2718-bucket/data/output.csv" 
    )
